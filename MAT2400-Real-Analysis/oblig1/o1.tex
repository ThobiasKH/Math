...
\begin{prob*}[3]
    ...
    \[
        ...
        \frac{dx}{dt}(t) \right|\right| \qquad \text{for} \ x 
        \in E_c
    \]
...
\begin{proof}[Proof of (a)]
    ...
    Now \textbf{Claim 1} follows nicely as we just saw that 
    the assumption that $||x(\alpha_c) - x(\beta_c)|| > 
    c ||\alpha_c - \beta_c||$ was wrong. 
    
    \textbf{Claim 3.}

    Notice that as $x \in E_c$ is continuous, and it's derivative 
    continuous, the average growth between $0$ and $1$ is 
    $||x_1 - x_0||$. By the Mean-Value-Theorem there must be 
    some point $t \in [0,1]$ such that the momentary growth in 
    that point is $||x_1 - x_0||$. 
    So $\frac{dx}{dt}$ must be at least $||x_1 - x_0||$. Therefore 
    we require $c \geq ||x_1 - x_0||$. 
\end{proof}
\begin{proof}[Proof of (b)]
    Suppose $(x_n)_{n\in\mathbb N}$ is a sequence in $E_c$. 
\end{proof}
