...
\subsection{Some practice problem}

\begin{prob}
    Work over $\mathbb Z$. Simplify as 
    far as possible: 
    \[
        (3a + 2b) \otimes n, 
    \]
    and show directly from 
    the tensor rules that 
    \[
        6 \otimes \overline 1 = 0
    \]
    in 
    \[
        \mathbb Z \otimes_{\mathbb Z} \mathbb Z/3\mathbb Z. 
    \]
    Then decide whether 
    \[
        2 \otimes \overline 1 
    \]
    is zero in the same tensor products. 
\end{prob}
\begin{proof}[Solution]
    We have 
    \begin{align*}
        (3a + 2b) \otimes n 
        &= 3a \otimes n + 2b \otimes n \\ 
        &= 3(a \otimes n) + 2(b\otimes n).  
    \end{align*}
    We have 
    \begin{align*}
        6 \otimes \overline 1 
        &= 2 \cdot 3 \otimes \overline 1 \\ 
        &= 1 \otimes (2 \cdot 3) \overline 1 \\ 
        &= 1 \otimes (2) \overline 0 \\ 
        &= 1 \otimes \overline 0 \\ 
        &= 0
    \end{align*}
    in $\mathbb Z \otimes \mathbb Z/3\mathbb Z$. 

    No we do not have $2\otimes \overline 1 = 0$. 
    ...
    \]
    via $a\times \overline b \mapsto a \overline b$ 
    so we get 
    \[
        2 \otimes \overline 1 
        \mapsto \overline 2 \neq \overline 0. 
...
\begin{prob}
    ...
    \[
        ...
        \alpha(f,g) = fg. 
    \]
    Prove $\alpha$ is bilinear. Use the universal 
    property to obtain 
    a linear map 
    \[
        \varphi : k[x] \otimes_k k[y] 
        \to k[x,y]
    \]
    satisftying 
    \[
    ...
    \]
    Finally argue that $\varphi$ is 
    an isomorphism by looking at the elements 
    \[
        x^i \otimes y^j. 
    \]
\end{prob}
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \alpha(f,ag) 
    \end{align*}
    so for fixed $g$ this is linear. 
    The procedure is the same for 
    showing linearity in the second argument. 

    For the tensor product we have a map 
    $\tau : k[x] \times k[y] \to k[x] \otimes_k 
    k[y]$ 
    for every bilinear map, such as $\alpha : 
    k[x] \times k[y] \to k[x,y]$, so that it factors uniquely 
    through a linear map 
    $\varphi : k[x] \otimes_k k[y] \to k[x,y]$ 
    such that 
    \[
        \begin{tikzcd}
            k[x] \times k[y] \arrow[r, "\alpha"]
            \arrow[dr, "\tau"] & 
            k[x,y] \\ 
            & k[x] \otimes_k k[y] \arrow[u, "\varphi"]
        \end{tikzcd}
    \] 
    commutes, meaning 
    $\alpha = \varphi \circ \tau$. Recalling that 
    $\alpha(f,g) = fg$, and 
    $\tau(f,g) = f \otimes g$, we know that 
    $\varphi(f \otimes g) = fg$. 

    The basis for $k[x] \otimes k[y]$ as an 
    $R$-algebra is $x^i \otimes y^j$ for $i,j \geq 0$ 
    and the basis for $k[x,y]$ is 
    \[
        \{1, x, y, x^2y, xy^2, \ldots\}
    \]
    so since elements in each 
    can be written in these bases we have the 
    correspondence: 
    \[
        \varphi\left(\sum_{i,j} 
        a_{ij} x^i \otimes y^j\right) 
        = \sum_{i,j} a_{ij}x^ib^j 
    \]
    both with finitely many $a_{ij}$ nonzero. 
\end{proof}
...
\end{prob}
\begin{proof}
    Let $\alpha : M \times R \to M$ by 
    \[
        \alpha(m,r) = rm. 
    \]
    This is a well defined, bilinear homomorphism for obvious 
    reasons (trust me bro). 

    Thus, by the universal property, there is a 
    $\varphi : M \otimes_R R \to M$ 
    making 
    \[
        \begin{tikzcd} 
            M \times R \arrow[dr, "\tau"] 
            \arrow[r, "\alpha"] & M \\ 
            & M \otimes_R R \arrow[u, "\varphi"]
        \end{tikzcd}
    \]
    commute, 
    where $(m,r) \overset\tau\mapsto m \otimes r$.

    So $\varphi$ is the homomorphism sending 
    $m \otimes r$ to $rm$. 

    Let $\psi : M \to M \otimes_R R$ by 
    \[
        \psi(m) = m \otimes 1. 
    \]
    Clearly, this is a homomorphism. 
    For the isomorphism argument observe the following. 

    Then $(\varphi \circ \psi)(m) = m$. 
    For the other direction we look at pure 
    tensors: 
    \[
        (\psi\circ \varphi)(m\otimes r) 
        = \psi(rm) = rm \otimes 1 = m\otimes r. 
    \]
...
\begin{prob}
    Consider 
    \[
        \mathbb Z/4\mathbb Z \otimes_{\mathbb Z} 
        \mathbb Z/6\mathbb Z.
    \]
    Define $\alpha : \mathbb Z/4\mathbb Z \times 
    \mathbb Z/6\mathbb Z \to \mathbb Z/2\mathbb Z$ by 
    \[
        \alpha(\overline a, \overline b) 
        = \overline{ab} \mod 2. 
    \]
    
    Show $\alpha$ is well-defined, 
    $\mathbb Z$-bilinear, derive 
    a homomorphism from the tensor product into 
    $\mathbb Z/2\mathbb Z$ and finally prove that 
    \[
        \overline 1 \otimes \overline 1 \neq 0. 
    \]
\end{prob}
\begin{proof}
    Clearly, $\alpha$ is well-defined since 
    taking the product of some integers $a,b$ and reducing 
    modulo $2$ yields a value in $\mathbb Z/2\mathbb Z$. 

    Next we check bilinearity. 
    \begin{align*}
        \alpha(\overline{a_1} + \overline{a_b}, b) 
        &= \overline{(a_1 + a_2)b} \mod 2 \\ 
        &= \overline{a_1b + a_2b} \mod 2 \\ 
        &= \overline{a_1b} + \overline{a_2b} \mod 2 \\ 
        &= \alpha(a_1, b) + \alpha(a_2,b). 
    \end{align*}
    The check for linearity in the second argument is analogous. 

    Let $\tau$ be the map which sends $(\overline a, \overline b) 
    \overset\tau\mapsto \overline a \otimes \overline b$. 

    Then there exists $\varphi : \mathbb Z/4\mathbb Z 
    \otimes_{\mathbb Z} \mathbb Z/6\mathbb Z \to 
    \mathbb Z/2\mathbb Z$ such that 
    \[
        \begin{tikzcd}
            \mathbb Z/4\mathbb Z \times \mathbb Z/6\mathbb Z 
            \arrow[dr, "\tau"] \arrow[r, "\alpha"] 
            & \mathbb Z/2\mathbb Z \\ 
            & \mathbb Z/4\mathbb Z \otimes_{\mathbb Z} 
            \mathbb Z/6\mathbb Z \arrow[u,"\varphi"]
        \end{tikzcd}
    \]
    commutes. 

    Then $\varphi$ is the homomorphism sending 
    $\overline a \otimes \overline b \overset\varphi\mapsto 
    \overline{ab}$ reduced modulo $2$. 

    Then 
    \begin{align*}
        \varphi(\overline 1 \otimes \overline 1) 
        &= \alpha(\overline 1,\overline 1) \\ 
        \varphi(\overline 1 \otimes \overline 1) 
        &= \overline{1 \cdot 1} \mod 2 \\ 
        \varphi(\overline 1 \otimes \overline 1)
        &= \overline 1 \mod 2 \\  
        \varphi(\overline 1\otimes \overline 1)   
        &\neq 0.
