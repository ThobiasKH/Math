...
% 1. Load a proper OpenType Math font first (Latin Modern Math is standard and works with the engine)
\setmathfont[
    Scale=1.25
]{latinmodern-math.otf}

% 2. Overwrite ranges to use the *Bold* version of JetBrains Mono for a visually heavier look.
% We use "Bold" in the font name to ensure we get the heavier weight you want.
\setmathfont[
  % Upright Latin and Digits (e.g., constants, functions like 'sin')
  range=\mathup/{latin, Latin, 0-9},
  % Italic Latin (e.g., variables)
  range=\mathit/{latin, Latin},
  % Bold Italic Latin (e.g., bold variables)
  range=\mathbfit/{latin, Latin},
  % Scaling to match text size
  Scale=1.5
]{JetBrains Mono Bold} % <<< Using 'JetBrains Mono Bold' here

% 3. Use a reliable font for specialized symbols like Blackboard Bold (\mathbb)
% Since you removed the previous line, we will ensure that \mathbb{R} works by including a reliable font.
% Let's use the Latin Modern Math font itself for these symbols, as it is already loaded.
